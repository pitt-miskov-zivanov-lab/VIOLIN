...
def kind_score(x, model_df, reading_df, graph, reading_cols,
               kind_values=kind_dict, attributes=atts_list, classify_scheme='1', mi_cxn='d'):
    """
    ...
    if len(list(set(['Reg Sign', 'Regulator Sign']) & set(reading_df.columns.tolist()))) > 0:
        ...
        if reading_df.at[x, reg_col][0].lower() in ['activate', 'positive', 'increase']:
            reg_sign = 'Positive'
        else:
            reg_sign = 'Negative'
    ...
    if 'Connection Type' in reading_df.columns:
        lee_cxn_type = reading_df.at[x, 'Connection Type']
    else:
        lee_cxn_type = "i"
    ...
    if len(attributes) > 0:
        ...
        att_cols = [reading_cols['target_' + z.lower().replace(" ", "_")] for z in attributes] + [
            reading_cols[reg_sign[0:3].lower() + '_source_' + z.lower().replace(" ", "_")] for z in attributes]
    ...
    if ((find_element("name", reading_df.at[x, reading_cols['target_name']],
                      reading_df.at[x, reading_cols['target_type']], model_df) != -1 or
         find_element("id", reading_df.at[x, reading_cols['target_id']], reading_df.at[x, reading_cols['target_type']],
                      model_df) != -1) and
            (find_element("name", reading_df.at[x, reading_cols[reg_sign[0:3].lower() + '_source_name']],
                          reading_df.at[x, reading_cols[reg_sign[0:3].lower() + '_source_type']], model_df) != -1 or
             find_element("id", reading_df.at[x, reading_cols[reg_sign[0:3].lower() + '_source_id']],
                          reading_df.at[x, reading_cols[reg_sign[0:3].lower() + '_source_type']], model_df) != -1)):

        # Find indices of regulated element (target) in model
        if (find_element("name", reading_df.at[x, reading_cols['target_name']],
                         reading_df.at[x, reading_cols['target_type']], model_df) != -1):
            model_t_indices = find_element("name", reading_df.at[x, reading_cols['target_name']],
                                           reading_df.at[x, reading_cols['target_type']], model_df)
        else:
            model_t_indices = find_element("id", reading_df.at[x, reading_cols['target_id']],
                                           reading_df.at[x, reading_cols['target_type']], model_df)

        # Find indices of regulator element (source) in model
        if (find_element("name", reading_df.at[x, reading_cols[reg_sign[0:3].lower() + '_source_name']],
                         reading_df.at[x, reading_cols[reg_sign[0:3].lower() + '_source_type']], model_df) != -1):
            model_s_indices = find_element("name",
                                           reading_df.at[x, reading_cols[reg_sign[0:3].lower() + '_source_name']],
                                           reading_df.at[x, reading_cols[reg_sign[0:3].lower() + '_source_type']],
                                           model_df)
        else:
            model_s_indices = find_element("id", reading_df.at[x, reading_cols[reg_sign[0:3].lower() + '_source_id']],
                                           reading_df.at[x, reading_cols[reg_sign[0:3].lower() + '_source_type']],
                                           model_df)
        ...
        for t_idx in model_t_indices:
            ...
            for s_idx in model_s_indices:
                ...
                if str(model_s_list) != "nan" and model_df.loc[s_idx, 'Variable'] in literal_eval(model_s_list):
                    ...
                    model_atts = [model_df.at[t_idx, att] for att in attributes] + [model_df.at[model_s_element, att]
                    ...
                    # If LEE ="I" and MI = "D":
                        ...
                        if compare_atts in [0, 1, 2]:
                            kinds.append(kind_values['weak corroboration2'])
                        # Else: LEE is a contradiction
                        elif compare_atts == 3:
                            kinds.append(kind_values['att contradiction'])
                ...
                # MI with Matched direction, Mismatched sign
                    ...
                    reg_index = literal_eval(model_df.loc[t_idx, opp_sign + " Regulators"]).index(
                        model_df.loc[s_idx, 'Variable'])
                    # Finding connection type
                    if (reg_sign + ' Connection Type') in model_df.columns.values.tolist() and \
                            str(model_df.loc[s_idx, opp_sign + ' Connection Type']).lower() not in ['', 'nan', 'NaN']:
                        # Connection type
                ...
                # MI with Mismatched direction, Matched sign
                elif (model_df.loc[s_idx, reg_sign + " Regulators"] != "nan" and model_df.loc[
                    t_idx, 'Variable'] in literal_eval(model_df.loc[s_idx, reg_sign + " Regulators"])):
                    reg_index = literal_eval(model_df.loc[s_idx, reg_sign + " Regulators"]).index(
                    ...
                    # List of model attributes to compare to reading attributes
                    model_atts = [model_df.at[s_idx, att] for att in attributes] + [model_df.at[model_reg_element, att]
                                                                                    for att in attributes]
                ...
                elif (model_df.loc[s_idx, opp_sign + " Regulators"] != "nan" and model_df.loc[
                    t_idx, 'Variable'] in literal_eval(model_df.loc[s_idx, opp_sign + " Regulators"])):
                    reg_index = literal_eval(model_df.loc[s_idx, opp_sign + " Regulators"]).index(
                        model_df.loc[t_idx, 'Variable'])
                    # Finding connection type
                ...
                # If model does not contain interaction - check for path
                    ...
                    kinds.append(
                        path_finding(model_df.loc[s_idx, 'Variable'], model_df.loc[t_idx, 'Variable'], reg_sign,
                                     model_df, graph, kind_values, lee_cxn_type, reading_atts, attributes,
                                     classify_scheme))
        ...
        # Weak Corroboration
        elif kind_values['weak corroboration1'] in kinds:
            kind = kind_values['weak corroboration1']
        elif kind_values['weak corroboration2'] in kinds:
            kind = kind_values['weak corroboration2']
        elif kind_values['weak corroboration3'] in kinds:
        ...
        # Flagged
        elif kind_values['flagged1'] in kinds:
            kind = kind_values['flagged1']
        elif kind_values['flagged2'] in kinds:
            kind = kind_values['flagged2']
        elif kind_values['flagged3'] in kinds:
            kind = kind_values['flagged3']
        elif kind_values['flagged4'] in kinds:
            kind = kind_values['flagged4']
        elif kind_values['flagged5'] in kinds:
    ...
    elif ((find_element("name", reading_df.at[x, reading_cols['target_name']],
                        reading_df.at[x, reading_cols['target_type']], model_df) == -1 and
           find_element("id", reading_df.at[x, reading_cols['target_id']],
                        reading_df.at[x, reading_cols['target_type']], model_df) == -1) and
          (find_element("name", reading_df.at[x, reading_cols[reg_sign[0:3].lower() + '_source_name']],
                        reading_df.at[x, reading_cols[reg_sign[0:3].lower() + '_source_type']], model_df) == -1 and
           find_element("id", reading_df.at[x, reading_cols[reg_sign[0:3].lower() + '_source_id']],
                        reading_df.at[x, reading_cols[reg_sign[0:3].lower() + '_source_type']], model_df) == -1)):
        kind = kind_values['full extension']
...
def score_reading(reading_df, model_df, graph, reading_cols,
    ...
    for x in range(reading_df.shape[0]):
        ...
        scored_reading_df.at[x,'Kind Score'] = kind_score(x,model_df,reading_df,graph,reading_cols,kind_values,attributes,classify_scheme,mi_cxn)
